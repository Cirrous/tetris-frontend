    ...
    <div class='w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0'>
      <div class='p-6 space-y-4 md:space-y-6 sm:p-8'>
        ...
        <form @submit.prevent='authStore.loginUser(form)' class='space-y-4 md:space-y-6'>
          ...
          <div>
            <button type='submit'
                    class='w-full flex justify-center items-center text-white bg-blue-600 hover:bg-blue-700 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center'
                    :disabled='authStore.isLoading'>
              Anmelden
              <div v-if='authStore.isLoading'
                   class='h-5 w-5 ms-2 animate-spin rounded-full border-4 border-solid border-current border-r-transparent text-white'></div>
            </button>
          </div>
          <p class='text-sm font-light text-gray-500'>
            Sie haben noch kein Konto?
            <router-link class='font-medium text-blue-600 hover:underline' to='/register'>Registrieren</router-link>
          </p>
        </form>
